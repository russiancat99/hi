...
public class MeteorClient implements ClientModInitializer {
    public static final String MOD_ID = "devx-client";
    public static final Identifier HANDSHAKE_ID = Identifier.of(MOD_ID, "handshake");
    ...
    static {
        ...
        // BUILD_NUMBER = MOD_META.getCustomValue(devx-client.MOD_ID + ":build_number").getAsString();
    }
    ...
    private void onGameJoin(GameJoinedEvent event) {
        ...
        if (mc.isIntegratedServerRunning() || (mc.getCurrentServerEntry() != null && mc.getCurrentServerEntry().isLocal())) {
            return;
        }

        // 2. Check for the DevX Handshake
        boolean isSupported = net.fabricmc.fabric.api.client.networking.v1.ClientPlayNetworking.canSend(HANDSHAKE_ID);

        if (!isSupported) {
            mc.execute(() -> {
                if (mc.world != null) {

                    net.minecraft.text.Text reason = net.minecraft.text.Text.of("§aDevX§r: Access Denied. Server not supported.");
                    // Help! Why doesn't it work???
                    mc.world.disconnect(reason);
                    mc.disconnect(net.minecraft.text.Text.of("§aDevX§r: Access Denied. Server not supported."));
                    // Nevermind I fixxed it!!
                }
            });
        }
